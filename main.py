        ...
        if message.chat.type == "private":
            if message.text == "/start":
                ...
                if id_user == "":
                    ...
                    db.dell_mess(message.chat.id, message.message_id)
                    db.send(message.chat.id, "⚡️", key.main())
                    db.send(message.chat.id, "<b>👾 С возвращением!</b> #https://t.me/Slivki_Logs", "")
                conn.close()
            ...
            if st == 0 or status == 'left':
                db.send(message.chat.id, "❗️Для работы с парсером, вам нужно подписаться на канал с новостями!#https://t.me/Slivki_Logs\n\nhttps://t.me/"+cfg.name_news, key.check_sub())
            else:
                conn = sqlite3.connect("users.db")
                c = conn.cursor()
                status = ''
    ...
    try:
        ...
        if call.data == "check_sub":
            ...
            for balance_user, in c.fetchall():
                pass
            conn.close()
            db.send(call.message.chat.id, "💰 Ваш баланс: <b>" + str(balance_user) + " руб.</b>\n\n🤖 Количество рефералов: #https://t.me/Slivki_Logs<b>" + str(count_ref)+ "</b>", key.prof())
